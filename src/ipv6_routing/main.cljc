(ns ipv6-routing.main
  "Kotodama WASM entrypoint for the ipv6_routing clean-room actor (L4) — Clojure port.
  ...
  facts (`ipv6_routing.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ipv6_routing")
(def tier "L4")
...
(def entity-specs
  [{:entity "Ipv6Header"   :plural "ipv6headers"   :id-prefix "ipv6rout_ipv"
    :fields [:version :trafficClass :flowLabel :payloadLength :nextHeader :hopLimit] :required [:version :trafficClass]
    :coerce {:version :int :trafficClass :int :flowLabel :int :payloadLength :int :nextHeader :int :hopLimit :int} :refs {}}
   {:entity "Icmpv6Message" :plural "icmpv6messages" :id-prefix "ipv6rout_icm"
    :fields [:type :code :checksum] :required [:type :code]
    :coerce {:type :int :code :int :checksum :int} :refs {}}
   {:entity "DestinationUnreachable" :plural "destinationunreachables" :id-prefix "ipv6rout_des"
    :fields [:type :code :checksum] :required [:type :code]
    :coerce {:type :int :code :int :checksum :int} :refs {}}
   {:entity "TimeExceeded" :plural "timeexceededs" :id-prefix "ipv6rout_tim"
    :fields [:type :code :checksum] :required [:type :code]
    :coerce {:type :int :code :int :checksum :int} :refs {}}
   {:entity "ParameterProblem" :plural "parameterproblems" :id-prefix "ipv6rout_par"
    :fields [:type :code :pointer] :required [:type :code]
    :coerce {:type :int :code :int :pointer :int} :refs {}}
   {:entity "NdpMessage" :plural "ndpmessages" :id-prefix "ipv6rout_ndp"
    :fields [:type :code :targetAddress] :required [:type :code]
    :coerce {:type :int :code :int} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"ipv6_routing.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ipv6_routing-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ipv6_routing-compat/ready)
