(ns ipv6-routing.main-test
  "Contract + behavioral test for the ipv6_routing-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "ipv6_routing-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'ipv6-routing.main-test)]
